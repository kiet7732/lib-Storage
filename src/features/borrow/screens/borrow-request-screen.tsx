...
import { borrowedItems, getBorrowedBook } from '@/services/library';
...
export function BorrowRequestScreen() {
  ...
  const borrowedItem =
    intent === 'renew'
      ...
      (bookId ? getBorrowedBook(bookId as string) : undefined)
      : undefined;
  ...
  useEffect(() => {
    if (bookId) {
      fetch(`http://localhost:8088/api/books/${bookId}`)
        .then(res => {
        ...
        .then(item => {
          setBook({
            ...
            copiesAvailable: item.quantity ?? 1,
            location: item.location || "Quầy trung tâm - Thư viện"
          });
        })
        .catch(() => setBook(null))
  ...
  const sheetCopy: SheetCopy =
    intent === 'renew'
      ...
      : {
        ...
        description: `Sách hiện đang có sẵn ${book?.copiesAvailable || 0 - 0} cuốn.`,
        badge: 'Sẵn sàng cho mượn',
        badgeTone: 'success',
        badgeIcon: 'check-circle',
        rows: [
          { label: 'Nhận sách tại', value: book?.location || 'Quầy số 2 - Thư viện trung tâm' },
          { label: 'Khung giờ nhận', value: 'Trong vòng 24 giờ tới' },
          { label: 'Hạn trả tạm tính', value: getFutureDateString(14) },
        ],
        noteTitle: 'LƯU Ý KHI NHẬN SÁCH',
        noteBody:
          'Bạn cần đến quầy trong vòng 24 giờ.',
  ...
  return (
    <AppBottomSheet
      dynamicHeight
      minHeight={layout.isCompact ? 320 : 380}
      onClose={handleCloseSheet}
      footer={undefined}
    >
      {loading ? (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingBottom: 60 }}>
          <AppText tone="muted">Đang tải thông tin sách...</AppText>
        </View>
      ) : !book ? (
        <View
          style={{
        ...
        >
          ...
          <View style={{ alignItems: 'center', gap: 6 }}>
            ...
            <AppText tone="muted" style={{ textAlign: 'center', marginBottom: 16 }}>
              Hãy quay lại danh sách và chọn lại đầu sách để tiếp tục thao tác.
            </AppText>
            <AppButton label="Đóng" iconName="close" onPress={handleCloseSheet} />
          </View>
        </View>
      ) : (
        <ScrollView
